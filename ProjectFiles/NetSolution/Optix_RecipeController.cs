...
public class Optix_RecipeController : BaseNetLogic
{
    #region CoT_Syntegon

    [ExportMethod]
    public void DownloadToPLCVars(string recipeName, NodeId sessionID, NodeId ResultVar)
    {
        UAVariable DownloadResultState = (UAVariable)InformationModel.Get(ResultVar);
        DownloadResultState.RemoteWrite(8);
        feedbackList.Clear();
        ApplyFromDB(recipeName, CopyErrorPolicy.StrictCopy);
        DownloadResultState.Value = Checkresult(sessionID, "Download Failed");
    }

    [ExportMethod]
    public void CopyRecipe(String recipeNameCopyFrom, String recipeNameNew, NodeId sessionID, NodeId ResultVar)
    {
        UAVariable CopyResultState = (UAVariable)InformationModel.Get(ResultVar);
        CopyResultState.RemoteWrite(8);
        feedbackList.Clear();

        SelectFromDB(recipeNameCopyFrom, CopyErrorPolicy.StrictCopy);
        SaveToDB(recipeNameNew, CopyErrorPolicy.StrictCopy);

        CopyResultState.Value = Checkresult(sessionID, "Copy Failed");

    }

    [ExportMethod]
    public void DeleteRecipe(String recipeName, NodeId sessionID, NodeId ResultVar)
    {
        UAVariable CopyResultState = (UAVariable)InformationModel.Get(ResultVar);
        CopyResultState.RemoteWrite(8);
        feedbackList.Clear();
        ...
        CopyResultState.Value = Checkresult(sessionID, "deletion resulted in fault");

    }

    [ExportMethod]
    public void SelectRecipe(String recipeName, NodeId sessionID)
    {
        feedbackList.Clear();
        SelectFromDB(recipeName, CopyErrorPolicy.StrictCopy);
        Checkresult(sessionID, "Selection Failed");
    }

    [ExportMethod]
    public void SaveRecipe(String recipeName,NodeId sessionID, NodeId ResultVar)
    {
        UAVariable saveResultState = (UAVariable)InformationModel.Get(ResultVar);
        saveResultState.RemoteWrite(8);
        feedbackList.Clear();

        LoadFromPLC(CopyErrorPolicy.StrictCopy);
        SaveToDB(recipeName, CopyErrorPolicy.StrictCopy);
        SelectFromDB(recipeName, CopyErrorPolicy.StrictCopy);

        saveResultState.Value = Checkresult(sessionID, "Save Failed");

    }



    private int Checkresult(NodeId sessionID, String operationName)
    {
        ...
        if (GetExecutionResult().message == String.Empty)
        {
            Log.Debug("RecipeController: " + operationName + "Sucsessfull");
            Logger.CreateAlarmAndLog("RecipeController: " + operationName + "Sucsessfull", MessageType.Info);
            return 6;
        }
        else
        {
            Cot_Dialog_RecipeFailiureInjection cot_Dialog_RecipeFailiureInjection = InformationModel.MakeObject<Cot_Dialog_RecipeFailiureInjection>("cot_Dialog_RecipeFailiureInjection");
            cot_Dialog_RecipeFailiureInjection.headerText = new LocalizedText("Recipehandling Fault", _locale);
            cot_Dialog_RecipeFailiureInjection.failureText = new LocalizedText(GetExecutionResult().message,_locale);
            UICommands.OpenDialog(_mainWindows, (DialogType)Project.Current.Get("UI/Screens/TemplateScreens/RecipeScreens/Dialogs/CoT_Dialog_RecipeFailure"), cot_Dialog_RecipeFailiureInjection.NodeId);
            Logger.CreateAlarmAndLog("RecipeController: " + GetExecutionResult().message);
            return 3;
        }
    }

    private (byte result, string message) GetExecutionResult()
    {
        if (feedbackList.Count == 0)
        {
            return (0, String.Empty);
        }
        (byte result, string message) highestFeedback = feedbackList.OrderByDescending(feedback => feedback.result).First();
        if (highestFeedback.result == 0 || highestFeedback.result == 1)
        {
            highestFeedback.message = String.Empty;
        }
        return highestFeedback;
    }

    List<(byte result, string message)> feedbackList = new List<(byte result, string message)>();
    private void SetFeedback(byte result, string message)
    {
        ...
        lock (lockObject)
        {
            feedbackList.Add((result, message));
        }
    }
